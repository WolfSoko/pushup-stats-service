 ...
 * failure mode behind every earlier "the notification logged push-ups"
 * report.
 *
...
export function parseReminderActionRequest(
  ...
  if (typeof token !== 'string' || token.length < 1 || token.length > 256)
    return null;
  // A reminder dispatched before this action was retired still carries its
  // button. Tapping it lands here and is refused like any other unknown
  // action rather than honoured against logic that no longer exists.
