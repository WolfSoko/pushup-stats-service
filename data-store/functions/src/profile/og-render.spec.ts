...
describe('buildOgTree', () => {
  ...
  it('Renders the stats string with the separator dot between streak and days', () => {
    // The combined stats line reads "N Reps · Streak M · D Tage".
    // The separator must always be present so the three stat groups
    // remain visually distinct regardless of the locale or number value.
    const text = flatten(buildOgTree(profile));
...
describe('_resetOgCachesForTesting', () => {
  it('is a callable no-arg function', () => {
    // Verify the export contract so tests can always clear module-level
    // caches without triggering TypeScript errors on the call site.
    expect(typeof _resetOgCachesForTesting).toBe('function');
    expect(() => _resetOgCachesForTesting()).not.toThrow();
  });

  it('Calling it twice in a row does not throw', () => {
    expect(() => {
