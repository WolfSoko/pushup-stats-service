...
describe('PublicProfileApiService', () => {
  ...
  describe('getProfile', () => {
    ...
    it('Invokes the callable with the function name getPublicProfile', () => {
      ...
      // The service must wire to the correct Cloud Function name so requests
      // are not silently swallowed by an undefined endpoint.
    ...
    it('Given the callable rejects with an Error that has no code, Then re-throws', async () => {
    ...
    it('Given the callable rejects with null, Then re-throws', async () => {
      const { service } = setup(async () => {
        throw null; // eslint-disable-line @typescript-eslint/no-throw-literal
      });

      await expect(service.getProfile('abcdef1234567890')).rejects.toBeNull();
    });
  });
});
