...
describe('exercisesByCategory', () => {
  ...
  it('returns the same reference on repeated calls (cached)', () => {
    const first = exercisesByCategory();
    const second = exercisesByCategory();
    expect(first).toBe(second);
  });

  it('contains no entries under the pushup category (pushups use the legacy catalog)', () => {
    const map = exercisesByCategory();
    const pushupEntries = map.get('pushup') ?? [];
    // Phase-0 catalog has no exercises under 'pushup' category —
    // those live in the legacy pushup-type.models.ts catalog.
    expect(pushupEntries.length).toBe(0);
  });
});

describe('EXERCISE_CATALOG — boundary values', () => {
  it('abs.situps has min=1 matching Firestore rule lower cap', () => {
    const def = findExerciseDefinition('abs.situps');
    expect(def?.min).toBe(1);
  });

  it('abs.situps has max=500 matching Firestore rule upper cap', () => {
    const def = findExerciseDefinition('abs.situps');
    expect(def?.max).toBe(500);
  });

  it('legs.squats has min=1 matching Firestore rule lower cap', () => {
    const def = findExerciseDefinition('legs.squats');
    expect(def?.min).toBe(1);
  });

  it('legs.squats has max=500 matching Firestore rule upper cap', () => {
    const def = findExerciseDefinition('legs.squats');
    expect(def?.max).toBe(500);
  });

  it('every catalog entry declares the reps measurement (Phase-0 only ships rep-based exercises)', () => {
    for (const def of EXERCISE_CATALOG) {
      expect(def.measurement).toBe('reps');
    }
  });

  it('every category has a positive order value', () => {
    for (const cat of EXERCISE_CATEGORIES) {
