...
- **Every promise-based request in `data-access`, `data-access-state` and `web` is tracked.** Wrap Firestore writes, transactions, one-off reads and callables in `PendingRequestsService.track()` (in `web`, callables already go through `CallableFunctionsService`); live listeners stay untracked. `auth`, `motivation` and `push` may not import `@pu-stats/data-access` and keep their own per-action loading state — their requests never show the global spinner. Details: [`docs/architecture.md`](docs/architecture.md#global-pending-request-indicator).
