...
- **Every promise-based request in `data-access`, `data-access-state`, `push` and `web` is tracked.** Wrap Firestore writes, transactions, one-off reads, Storage calls and callables in `PendingRequestsService.track()` (in `web`, callables already go through `CallableFunctionsService`); live listeners stay untracked. `auth` and `motivation` may not import `@pu-stats/data-access` and keep their own per-action loading state — their requests never show the global spinner. Details: [`docs/architecture.md`](docs/architecture.md#global-pending-request-indicator).
