...
- **`toSignal` lag in route guards:** Promise-based `CanActivateFn` guards are NOT reactive — signal reads after `await` are not tracked. `publicOnlyGuard` (and any guard backed by `toSignal`) MUST call `await firebaseAuth.authStateReady()` and then read `firebaseAuth.currentUser` synchronously as the authoritative source, mirroring the `authGuard` pattern. Relying solely on `auth.isAuthenticated()` causes a race where authenticated users slip through to `/login` or `/register` during the settling window, triggering mid-flow redirects.
- **`toSignal` lag in post-auth hooks:** `AuthService.runPostAuthHooks()` must prefer `this.authAdapter.currentUser` (synchronous) over the signal-based `this.user()` — after `linkWithEmail`/`linkWithGoogle`, the signal may still hold the stale anonymous user due to same-UID upgrade microtask delay.
- **Auth store methods return `boolean`:** `login()`, `signInWithEmail()`, `signUpWithEmail()`, `upgradeWithGoogle()` return `Promise<boolean>` (true = success, false = failure with error already set). Callers MUST use the returned boolean — do NOT check `!authStore.error()` as a success signal, as it races against signal settling.
