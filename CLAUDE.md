...
- **Every promise-based request is tracked.** Wrap Firestore writes, one-off reads and callables in `PendingRequestsService.track()` (in `web`, callables already go through `CallableFunctionsService`); live listeners stay untracked. This drives the global loading spinner — details: [`docs/architecture.md`](docs/architecture.md#global-pending-request-indicator).
