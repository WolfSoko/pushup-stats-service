...
describe('PublicProfilePageComponent', () => {
  ...
  describe('SEO and OG image', () => {
    it('Sets a dynamic OG image URL pointing to the ogProfile function', async () => {
      await setup({ resolve: sampleProfile });

      expect(seoMock.update).toHaveBeenCalled();
      const args: unknown[] = seoMock.update.mock.calls.at(-1)!;
      // The fourth argument carries OG extras including imageUrl.
      const ogExtras = args[3] as { imageUrl?: string; imageAlt?: string } | undefined;
      expect(ogExtras?.imageUrl).toContain('ogProfile');
      // The UID must be URL-encoded in the query string.
      expect(ogExtras?.imageUrl).toContain(encodeURIComponent(sampleProfile.uid));
    });
    ...
    it('Calls seo.update for the not-found state so canonical stays consistent', async () => {
      await setup({ resolve: null });

      expect(seoMock.update).toHaveBeenCalled();
      // The not-found branch must NOT pass OG extras (there is no profile
      // image to render), so callers either receive undefined or an object
      // without imageUrl.
      const args: unknown[] = seoMock.update.mock.calls.at(-1)!;
    ...
    it('Includes the canonical path /u/:uid in the SEO call for a ready profile', async () => {
      ...
      const args: unknown[] = seoMock.update.mock.calls.at(-1)!;
      // Third arg is the canonical path.
      expect(args[2]).toBe('/u/abcdef1234567890');
    });
  });

  describe('Profile stats rendering', () => {
    it('Renders bestSingleEntry when present', async () => {
      await setup({ resolve: sampleProfile });

      // bestSingleEntry = 50 in the sample. The value is formatted through
      // the number pipe so strip locale separators before matching.
      const text = (fixture.nativeElement as HTMLElement).textContent ?? '';
      expect(text.replace(/[,.\s]/g, '')).toContain('50');
    });

    it('Shows the share button when a profile is loaded', async () => {
      await setup({ resolve: sampleProfile });
    ...
    it('Does not show the share button when not-found', async () => {
      await setup({ resolve: null });
