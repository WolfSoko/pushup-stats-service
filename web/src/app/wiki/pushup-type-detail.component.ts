      ...
      <article class="detail-page">
        <header class="detail-header">
          <a
            ...
            class="back-link"
            i18n="@@wiki.pushupType.detail.back"
          >
        ...
        <footer class="detail-footer">
          <a
            mat-stroked-button
            routerLink="/wiki/liegestuetz-typen"
            i18n="@@wiki.pushupType.detail.toList"
            >Alle Liegestütztypen</a
          >
          <a
            mat-flat-button
            color="primary"
            routerLink="/training-plans"
            i18n="@@wiki.pushupType.detail.cta"
...
export class PushupTypeDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.type = found;
    const localized = localizePushupType(found, this.locale);
    ...
    // best-effort hint for Bing/Yandex without per-locale arrays.
    this.meta.updateTag({
      name: 'keywords',
      content: [...found.keywordsDe, ...found.keywordsEn].join(', '),
    });
    this.injectJsonLd(found);
  }

  private injectJsonLd(type: PushupTypeInfo): void {
    ...
    script.setAttribute('data-pushup-type-ld', '1');
    script.textContent = JSON.stringify(jsonLd);
    head.appendChild(script);
  }

  private removeJsonLd(): void {
    this.document.head?.querySelector('script[data-pushup-type-ld]')?.remove();
  }
}
