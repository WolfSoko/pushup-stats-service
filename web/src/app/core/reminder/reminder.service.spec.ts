...
describe('isInQuietHours', () => {
  ...
  function dateAt(timezone: string, isoDate: string, hhmm: string): Date {
  ...
  it('returns true during a simple overnight window (22:00–07:00)', () => {
    ...
    expect(isInQuietHours([{ from: '22:00', to: '07:00' }], 'UTC', now)).toBe(true);
  });
  ...
  it('returns false outside overnight window (12:00 UTC)', () => {
    ...
    expect(isInQuietHours([{ from: '22:00', to: '07:00' }], 'UTC', now)).toBe(false);
  });
  ...
  it('returns true during a simple same-day window (12:00–13:00)', () => {
    ...
    expect(isInQuietHours([{ from: '12:00', to: '13:00' }], 'UTC', now)).toBe(true);
  });

  it('returns false before same-day window (11:59 UTC)', () => {
    const now = new Date('2026-03-23T11:59:00Z');
    expect(isInQuietHours([{ from: '12:00', to: '13:00' }], 'UTC', now)).toBe(false);
  });
  ...
  it('uses the provided now parameter instead of current time', () => {
    ...
    expect(isInQuietHours([{ from: '22:00', to: '07:00' }], 'UTC', pastNight)).toBe(true);
  });
  ...
  it('handles same from and to (always-quiet degenerate case)', () => {
    ...
    expect(isInQuietHours([{ from: '10:00', to: '10:00' }], 'UTC', now)).toBe(false);
  });
});
