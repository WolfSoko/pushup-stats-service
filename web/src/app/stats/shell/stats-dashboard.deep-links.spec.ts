...
describe('registerDashboardDeepLinks', () => {
  ...
  it('should ignore a leftover snooze param', () => {
    // given a deep link from a reminder sent before snooze was retired
    const { openCreateDialog, navigate } = setup({ snooze: '30', log: '1' });

    // then the log dialog still opens and the stray param changes nothing
    expect(openCreateDialog).toHaveBeenCalledTimes(1);
