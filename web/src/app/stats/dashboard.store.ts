...
export const DashboardStore = signalStore(
  withState({
    dailyGoal: 100,
    todayQuote: null as string | null,
  }),
  withProps(() => ({
    _api: inject(StatsApiService),
    _userConfigApi: inject(UserConfigApiService),
    _user: inject(UserContextService),
    _live: inject(PushupLiveService),
    _adsConfig: inject(AdsConfigService),
    _motivationService: inject(MotivationQuoteService),
  })),
  withProps((store) => ({
  ...
  withComputed((store) => {
    ...
    const adClient = computed(() => store._adsConfig.adClient());
    const adSlotDashboardInline = computed(
      () => store._adsConfig.dashboardInlineSlot()
    );
    const dashboardInlineAdsEnabled = computed(
      () => store._adsConfig.dashboardInlineEnabled()
    );
  ...
  withMethods((store) => ({
    ...
    async loadQuote(): Promise<void> {
      const quote = await store._motivationService.getTodayQuote({
        userId: store._user.userIdSafe(),
      });
      patchState(store, { todayQuote: quote });
    },
