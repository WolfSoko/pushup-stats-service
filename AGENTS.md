...
Long-form content lives in markdown files with YAML frontmatter under `content/`, one file per locale. A build-time generator (`tools/src/generate-content.mjs`, Nx target `tools:generate-content`) renders the markdown bodies to HTML and emits:

- `web/src/app/blog/generated/<slug>.<lang>.ts` — one TS module per blog post per locale, each exporting a single `POST: BlogPost` constant. Per-post files give reviewers a focused diff when a single post changes and avoid one giant file growing without bound.
- `web/src/app/blog/generated/index.ts` — barrel that imports every per-post module and re-exports them as `GENERATED_BLOG_POSTS`.
...
- Adding a third locale = drop in `<folder>/fr.md`. The generator scans every `<lang>.md` in the folder automatically.
...
- `localizePushupType()` prefers the markdown override and falls back to the legacy parallel `*En` fields. All 13 catalog entries now have markdown overrides, so the fallback is dead code kept for type safety until a follow-up cleanup removes the `*En` fields from `PushupTypeInfo`.

**Migration status.** All 22 blog posts and all 13 push-up types have been moved into markdown. `web/src/app/blog/blog-posts.data.ts` is now a thin wrapper that re-exports `GENERATED_BLOG_POSTS` as `BLOG_POSTS`. The sitemap generator (`tools/src/generate-sitemap.js`) reads markdown frontmatter directly via the `yaml` package, so it runs without depending on the generated TS modules.
