...
- **Two rep detectors, one state machine.** The pose counter feeds joint angles into `RepStateMachine`; the proximity counter (phone lying face-up under the user) feeds the brightness swing of the frame through `BrightnessEnvelope` onto the same 0–180° scale (`RepThresholds`), so dwell/gap/hysteresis logic exists once. The proximity path needs no MediaPipe; `ExerciseAngleProfile.proximityCapable` (`supportsProximityCount`) says where it makes sense — pushups and squats, not sit-ups or pull-ups — and `AutoCountDialogComponent` switches between the two via its mode toggle on the same camera stream, falling back to pose for an unsupported exercise.
