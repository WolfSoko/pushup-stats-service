...
`getFriendsLeaderboard` ranks the viewer and their confirmed friends for one **metric** and period (`daily` / `week` / `month` / `allTime`). The metric is the answer to "how do friends with different favourite exercises compare fairly": `days` (training days, the default) and `streak` (the live streak, dead once the last entry is older than yesterday) ignore the exercise entirely; `reps` is one exercise's count for those who want to race, picked from the rep-counted part of `EXERCISE_CATALOG`. It deliberately does **not** read the public leaderboard snapshot: that is a precomputed top-N list, and friends are an arbitrary set who will mostly not be in it. Values come from the aggregates via two `getAll` calls (configs + `userStats/{uid}` for days and streak, `…/perExercise/{exerciseId}` for reps), with the same stale-bucket rule as everywhere else — a `dailyKey` that is not today counts as zero. The one exception is training days within a week or month: the aggregates keep day counts only for all time, so `friends/training-days-read.ts` counts distinct Berlin dates from each participant's entries with one projection query per user (`select('timestamp')`, started a day early because stored timestamps carry their own offset). The client keeps the chosen comparison in `FriendsStore.boardComparison`; the dashboard preview uses the same one.
