...
- **A section invented later starts at `friends`.** `FRIENDS_UNLESS_CHOSEN` in `profile-visibility.models.ts` lists them (today: `recent`, the last ten workouts). The legacy master switch meant "show my stats", not "show every kind of data added afterwards", so a new section never inherits it — the owner publishes it with its own switch or not at all. `getPublicProfile` also skips the extra Firestore query unless the viewer may see the section.
