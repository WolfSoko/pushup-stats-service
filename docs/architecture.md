...
- **The session starts from two places.** The dashboard's plan banner and the plan page's today card both link `/training-plans/:slug/session` under the same condition (`offersSession`): the plan is active, today isn't a rest day, and something on it is still open.
