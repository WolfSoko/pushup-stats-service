...
- **Every promise-based request goes through `pending.track(promise)` at the API-service layer:** Firestore writes (`setDoc`/`updateDoc`/`deleteDoc`/`batch.commit()`), one-off reads (`getDoc`/`getDocs`) and callables. In `web`, callables are covered centrally by `CallableFunctionsService`; `HttpClient` requests by `pendingRequestsInterceptor`.
