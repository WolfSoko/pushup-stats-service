...
- **A section invented later starts at `off`.** `OFF_UNLESS_CHOSEN` in `profile-visibility.models.ts` lists them (today: `recent`, the last ten workouts, and `plan`, the running training plan). Neither the legacy master switch nor a friendship accepted last month said anything about a kind of data that did not exist then, so a new section inherits neither — the owner publishes it with its own switch or not at all, and sees it on their own page meanwhile. `getPublicProfile` skips the extra Firestore read unless the viewer may see the section.
