...
`updateTrainingStatsOnEntryWrite` (`functions-training-stats.ts`) folds each entry write in as remove-old + add-new (`nextTrainingStats`), deduped by event id like `userXp`. A missing doc, one below `TRAINING_STATS_VERSION`, or a removal that may have taken the best entry is rebuilt from all of the user's entries instead. XP is priced at today's rate (`trainingLineOf`), not the ledger's frozen one, because the ledger line may not exist yet; both sides of an edit are priced alike, so they cancel out.

Readers use `readTrainingStats`, which falls back to computing the aggregate in memory from the entries for a user who has not trained since it shipped. Read paths never write it. The public profile (all-time cards, heatmap, OG image), the friends board (`days`, `streak`) and the dashboard all show these numbers. The dashboard computes them from its live entries through the very same fold (`summarizeTraining`), so the numbers match without a round trip. It lives under `userStats/{uid}`, so the account-deletion purge takes it along.
