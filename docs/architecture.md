...
2. `aggregateXpOnLedgerWrite` (`userXp/{uid}/xpLedger/{entryId}`) folds the change into `userXp/{uid}` — `total`, `level`, `dailyXp`/`weeklyXp`/`monthlyXp` with their period keys (same stale-key rule as `userStats`), `byExercise`. A missing doc or one below `USER_XP_VERSION` is rebuilt from the whole ledger. It then awards **level badges** (`LEVEL_MILESTONES` 5…100) and **variety badges** (XP in 3/5/8 categories) by appending to `userAchievements/{uid}.earned` (whole-doc rewrite, never merge) plus an inbox notification.
