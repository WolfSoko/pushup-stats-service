...
- **`sitemap.xml`** (`web/public/sitemap.xml`) is auto-generated by `tools/src/generate-sitemap.js` (Nx target: `pnpm nx run tools:generate-sitemap`). The generator runs automatically before every `web:build`. Sources:
  ...
  - Exercise wiki & pushup type detail pages → auto-extracted from `libs/stats/src/lib/models/exercise-wiki.models.ts` and `pushup-type.models.ts`.
  - Locale fan-out: pushup types, training plans, exercise wiki, and blog posts emit **one `<loc>` per `SUPPORTED_LOCALES` entry**, each with a complete hreflang alternates set. The locale list itself is parsed from `web/src/server-locale-redirect.ts` so the sitemap, the SSR redirect, and the SEO service stay in sync. Static routes are still emitted once under `/de` with hreflang alternates.
