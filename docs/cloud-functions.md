...
- **training/:** the cross-exercise training aggregate — entry → line mapping (`lines.ts`), reads with the in-memory fallback (`stats-read.ts`), backfill planning (`backfill-plan.ts`); the trigger is `functions-training-stats.ts`, the admin migration `backfillTrainingStats` lives in `functions-training-backfill.ts`, design in [`architecture.md`](architecture.md#cross-exercise-training-aggregate)
