...
- **`whenStable()` hangs while a `resource()` is pending:** a component-owned `resource` whose loader promise never settles registers a pending task, so `await fixture.whenStable()` waits until the 30 s test timeout. To assert the loading state, only call `fixture.detectChanges()`; resolve the deferred promise before the first `whenStable()`.
...
## Emulator E2E (`web-e2e-emulator`)

- **Sign in through the UI, seed everything else through the Admin SDK.** `support/backend.ts` creates the Auth record and the `userConfigs` document directly; the spec then signs that account in on the login form. Seeding the session instead would skip the post-auth hooks (profile sync, invite claim) that half the flows depend on, and driving registration to set up a friends test makes the friends test fail when registration breaks.
- **Give every spec its own accounts.** The suite never wipes the emulator between specs — `uniqueIdentity()` hands out a fresh address per account instead. Wiping would serialize the suite against shared state and would also delete the evidence after a failure.
- **Signal forms drop a `fill()` that lands before hydration.** The login and register pages are server-rendered; a value typed into the not-yet-hydrated form is silently lost and the submit button stays disabled forever. Re-enter the value inside `expect(...).toPass()` and use the button enabling as the proof it arrived (`LoginPage.fillCredentials`, `RegisterPage.fillEmail`).
- **Wait for the write to come back before driving the next step.** What a session's next set prescribes is `target − logged`, and `logged` arrives via the live `exerciseEntries` mirror, not from the click. Clicking at machine speed logs the previous set's reps a second time. The session's own "N von M Sätzen erledigt" line is the signal that the write landed.
- **The friends page re-reads on becoming visible, not on a timer.** Everything on it is someone else's doing. A request that lands while the page is already open only shows up on the next visit, so assertions reload the page (`FriendsPage.expectIncomingCount`).
- **The invite link points at the production origin.** `buildInviteUrl` hardcodes `https://pushup-stats.com`, so a test cannot follow it — pull the `fi` token out and rebuild the path against the origin under test (`invitePathFor`). Redemption then happens in a post-auth hook with nothing on screen marking it done; the parked token in `localStorage` being dropped is the observable end of it (`waitForInviteRedeemed`).
- **A dialog over the page hides it from role locators.** Material marks everything behind an open dialog `aria-hidden`, and that is the tree `getByRole` reads — the element is still visible and still in `innerText`, but the locator resolves to nothing. `FeatureAnnouncementService` opens the "what's new" walkthrough on the dashboard for any account that has not seen it, and only once the user config has arrived: locally the assertion wins that race, against a real backend it loses. Page objects therefore dismiss the overlay and look again (`DashboardPage.expectLoaded`, `support/overlay.ts`) instead of asserting once.
- **Against staging every in-app path needs the locale prefix.** Hosting only rewrites `/<locale>{,/**}`, so `/login` is a 404 there. The prefix cannot live in `baseURL` either — Playwright resolves a leading-slash `goto()` against the origin and drops the base URL's path. It belongs in the page objects (`appPath()` in `support/routes.ts`).
- **A CI-only failure has to explain itself in the job log.** The staging target never runs locally, and its Playwright report is an artifact somebody has to download first. The `page` fixture therefore prints the URL, the visible text and the browser's complaints when a test ends badly (`support/diagnostics.ts`), with query values redacted — an invite token travels in `?fi=`, and the Actions log is readable by anyone who can see the repo.
- **Entry timestamps carry no seconds.** `nowLocalIsoTimestamp()` stops at minutes, so any comparison against a millisecond-precision timestamp (`dayActivatedAt`) has to floor to the minute — otherwise the first set of a session started right after activating a plan is dropped. Fixed in `planExerciseLoggedTotal`; worth remembering wherever the two timestamp shapes meet.
