...
- **Angular template (HTML) rules are not linted.** `@angular-eslint/template` needs a custom parser, which oxlint does not support. `banana-in-box`, template `eqeqeq`, `no-negated-async`, `prefer-control-flow` and the template a11y rules therefore no longer run in CI.
