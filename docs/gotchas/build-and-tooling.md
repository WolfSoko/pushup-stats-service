...
2. **Listed in `ignorePatterns` of `.oxfmtrc.json`.** lint-staged runs `oxfmt` on commit; reformatting a generated file makes the committed bytes differ from the generator's output, which produces the same per-machine drift as (1) plus a large spurious diff after every local build.
...
Linting is oxlint (`.oxlintrc.json` at the root, no per-project configs) and formatting is oxfmt (`.oxfmtrc.json`). `@nx/oxlint` infers a `lint` target (`oxlint .` in the project root) for every project with lintable files; there is nothing to declare in `project.json`. Nx detects `.oxfmtrc.json` and routes `nx format` and generator `formatFiles` through oxfmt.

- **ESLint rules that oxlint has no native port for run as JS plugins** (`jsPlugins` in `.oxlintrc.json`): `@nx/enforce-module-boundaries` via `@nx/oxlint/boundaries-plugin`, the `@angular-eslint` TypeScript rules (selectors, lifecycle, `prefer-inject`, …) via `@angular-eslint/eslint-plugin`, and `eslint-plugin-playwright` for `web/web-e2e`. That is why `@angular-eslint/eslint-plugin`, `eslint-plugin-playwright` and `@typescript-eslint/utils` are still devDependencies although ESLint itself is gone. Oxlint's JS-plugin API is alpha and outside its semver policy — expect an oxlint bump to occasionally need a plugin bump.
- **Angular template (HTML) rules still run through ESLint.** `@angular-eslint/template` needs a custom parser, which oxlint cannot load, so the root `eslint.config.mjs` lints `*.html` only (recommended + accessibility rules). `@nx/eslint/plugin` (restricted to the `html` extension) infers a `lint-templates` target for every project that owns a template (currently `web`, `libs/auth`, `libs/quick-add`; new ones are picked up automatically), and `targetDefaults.lint.dependsOn` makes every `lint` run pull it in — `nx affected -t=lint` and `pnpm nx lint web` cover both. Do not put TypeScript rules into `eslint.config.mjs`; oxlint owns them.
- **The boundaries rule needs a cached project graph.** Run through Nx (`pnpm nx lint <project>`) and it is always there. A bare `pnpm exec oxlint` in a fresh clone prints "No cached ProjectGraph is available" once per file and skips the rule until any Nx command has built the graph.
- **Type-aware rules are on** (`options.typeAware` in `.oxlintrc.json`, backed by `oxlint-tsgolint`). `no-floating-promises` is the one that bites most: a fire-and-forget `router.navigate(...)` or async store call needs an explicit `void`. Two rules are tuned for this codebase: `typescript/unbound-method` is off in spec files (`expect(mock.method)` is the whole point there) and `typescript/no-base-to-string` is off because the Cloud Functions and auth code coerce loosely typed Firestore/Express/Firebase-error values with `String(x ?? '')` on purpose.
- **oxlint exits 1 when it is handed no lintable file** ("No files found to lint"). lint-staged therefore scopes `oxlint --fix` to script extensions; a bare `'*'` glob makes every commit that stages only manifests, JSON or Markdown fail the pre-commit hook. oxfmt has `--no-error-on-unmatched-pattern` for the same situation.
