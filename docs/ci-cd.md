...
- **Critical paths against staging:** the `build_and_preview` job in `.github/workflows/firebase-hosting-pull-request.yml` runs the same `web-e2e-emulator` specs a second time (`e2e-staging`), against the preview channel it just deployed and the real staging project — deployed rules, real callables, real composite indexes, none of which the emulator run can vouch for. It reuses the keyless WIF credential the deploy steps already set up, so no extra secret is involved, and it is gated to same-repo PRs like the rest of that job. The accounts a run creates are deleted again by `support/cleanup.ts`; see [`web/web-e2e-emulator/README.md`](../web/web-e2e-emulator/README.md).
