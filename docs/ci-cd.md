...
- **Critical-path E2E (`e2e-emulator` job):** a second Playwright project, `web-e2e-emulator`, covers the flows that need a backend — registering, signing in, adding a friend and answering the request, and working through a training session. It runs against the Firebase emulators (`data-store:serve-e2e`) with the app built by `web:build:development-emulator` on port 4302 (`web:serve-e2e-emulator`), so the real `firestore.rules` and the real callables are exercised. It runs in its own `ubuntu-latest` job rather than on the Nx Cloud agents: the Firestore emulator is a JAR and needs Java, and atomizing it would boot one emulator per shard for a suite that takes about a minute. The distributed `e2e` job therefore passes `--exclude=web-e2e-emulator`. Details: [`web/web-e2e-emulator/README.md`](../web/web-e2e-emulator/README.md).
- **Functions in the emulator:** `data-store:serve-e2e` first runs `tools/src/link-functions-emulator-deps.mjs`. The emulator loads `data-store/functions-dist`, the esbuild bundle, whose generated package.json has no installed dependencies — without the link firebase-tools drops every callable with "Failed to find location of Firebase Functions SDK" and the friends specs fail against a backend that is not there. `firebase deploy` never uploads node_modules, so the link cannot reach production.
- **Critical paths against staging:** the `build_and_preview` job in `.github/workflows/firebase-hosting-staging-preview.yml` runs, once a preview has been requested (see [PR previews on request](#pr-previews-on-request)), the same `web-e2e-emulator` specs a second time (`e2e-staging`), against the preview channel it just deployed and the real staging project — deployed rules, real callables, real composite indexes, none of which the emulator run can vouch for. It reuses the keyless WIF credential the deploy steps already set up, so no extra secret is involved. The workflow itself trusts the branch it is dispatched on; the same-repo and write-access gate lives in the dispatcher (`trigger_preview`), see below. The accounts a run creates are deleted again by `support/cleanup.ts`; see [`web/web-e2e-emulator/README.md`](../web/web-e2e-emulator/README.md). Because staging is one shared project, the whole workflow runs under a `concurrency` group — a second PR would otherwise deploy new rules or functions into the project while this run is testing the ones it just deployed. Note that the specs address the app under its locale prefix there (`/de/...`): Hosting only rewrites `/<locale>{,/**}`, so a locale-less path is a 404 on the preview channel.
...
- **PR Previews** (`.github/workflows/firebase-hosting-pull-request.yml` + `firebase-hosting-staging-preview.yml`): Full staging deployment **on request only** — tick the checkbox in the bot comment on the PR. See [PR previews on request](#pr-previews-on-request) and [Staging Environment](#staging-environment) below.
...
- The PR-preview workflow deploys its hosting channel with `firebase hosting:channel:deploy` and writes the preview URL into its request comment via `gh api` (replacing `FirebaseExtended/action-hosting-deploy`, which requires a key).

## PR previews on request

Staging is one shared project and a preview run (deploy + critical-path suite) blocks it for up to 50 minutes, so previews are not deployed automatically. Two workflows share the job:

- **Request comment.** When a same-repo, non-Dependabot PR is opened, the `request_preview` job in `.github/workflows/firebase-hosting-pull-request.yml` posts one comment (marker `<!-- staging-preview-request -->`) with the checkbox `- [ ] Preview auf Staging deployen`. Fork and Dependabot PRs get no comment: their `GITHUB_TOKEN` is read-only and they would not be deployed anyway.
- **Ticking the box** edits the comment, which fires `issue_comment: edited`. `trigger_preview` runs only when that edit comes from the bot's own comment, carries the marker and the ticked box. It unticks the box straight away (so it can be ticked again for a fresh deploy), resolves the PR head via `gh pr view`, refuses cross-repository heads and requesters without write access, and dispatches `firebase-hosting-staging-preview.yml` on the PR branch. It never checks out code: an `issue_comment` job runs with secrets, and a PR-head checkout there is what CodeQL's untrusted-checkout rule flags.
- **Deploy.** `firebase-hosting-staging-preview.yml` (`workflow_dispatch`, inputs `pr_number` + `comment_id`) runs on the PR branch like a push-triggered workflow would: build, deploy of functions/rules/indexes, hosting channel `pr-<n>`, then `e2e-staging` against that channel. The result (URL, commit, E2E outcome, run link) is written back into the same comment's `_Status:_` line, so the PR thread stays at one comment per PR; the status updates are best-effort and never fail the deploy. A new push does **not** redeploy — tick the box again; while a run for the branch is still queued or in progress, a new tick only points to that run instead of dispatching a second one. The workflow can also be started by hand from the Actions tab (leave `comment_id` empty for no status updates).
- **Branch requirements.** The dispatcher runs the workflow file from `main` (that is how `issue_comment` works), the deploy runs the PR branch's own copy — so changes to the deploy steps are exercised by the PR that makes them, but a PR branch must contain `firebase-hosting-staging-preview.yml` (merge `main` into branches that predate it).

## Staging Environment

A separate Firebase project (`pushup-stats-staging-867b7`) provides full isolation for PR previews:

- **PR workflow deploys** (on request, see above): Hosting preview + Cloud Functions + Firestore rules & indexes to the staging project.
...
- **Deploy authentication:** keyless via Workload Identity Federation (`vars.WIF_PROVIDER_STAGING` / `vars.WIF_DEPLOY_SA_STAGING`, set by `infra/setup-wif.sh`) — no service-account JSON secret. The former `FIREBASE_SERVICE_ACCOUNT_PUSHUP_STATS_STAGING` secret is obsolete and can be deleted.
