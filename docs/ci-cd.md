...
- **Critical paths against staging:** the `build_and_preview` job in `.github/workflows/firebase-hosting-staging-preview.yml` runs, once a preview has been requested (see [PR previews on request](#pr-previews-on-request)), the same `web-e2e-emulator` specs a second time (`e2e-staging`), against the preview channel it just deployed and the real staging project — deployed rules, real callables, real composite indexes, none of which the emulator run can vouch for. It reuses the keyless WIF credential the deploy steps already set up, so no extra secret is involved, and it is gated to same-repo PRs like the rest of that job. The accounts a run creates are deleted again by `support/cleanup.ts`; see [`web/web-e2e-emulator/README.md`](../web/web-e2e-emulator/README.md). Because staging is one shared project, the whole workflow runs under a `concurrency` group — a second PR would otherwise deploy new rules or functions into the project while this run is testing the ones it just deployed. Note that the specs address the app under its locale prefix there (`/de/...`): Hosting only rewrites `/<locale>{,/**}`, so a locale-less path is a 404 on the preview channel.
